...
class setupPheno:
    ...
    **Attributes:**
    ...
    def __init__(self, model="Skyrme", param="SLY5"):
        ...
        if model.lower() == "skyrme":
            ...
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/pheno/Skyrme/" + param + "-SM.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/pheno/Skyrme/" + param + "-NM.dat"
            )
            if nuda.env.verb:
            ...
            self.sm_den, self.sm_kfn, self.sm_e2a, a, self.sm_pre, self.sm_cs2 = (
                np.loadtxt(
                    file_in1, usecols=(0, 1, 2, 3, 4, 5), comments="#", unpack=True
                )
            )
            self.nm_den, self.nm_kfn, self.nm_e2a, a, self.nm_pre, self.nm_cs2 = (
                np.loadtxt(
                    file_in2, usecols=(0, 1, 2, 3, 4, 5), comments="#", unpack=True
                )
            )
        ...
        #
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/pheno/ESkyrme/" + param + "-SM.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/pheno/ESkyrme/" + param + "-NM.dat"
            )
            if nuda.env.verb:
                print("Reads file1:", file_in1)
            if nuda.env.verb:
            ...
            #: Attribute providing additional notes about the data.
            self.note = "write here notes about this EOS."
            self.sm_den, self.sm_e2a, self.sm_pre = np.loadtxt(
                file_in1, usecols=(0, 1, 2), comments="#", unpack=True
            )
            self.nm_den, self.nm_e2a, self.nm_pre = np.loadtxt(
                file_in2, usecols=(0, 1, 2), comments="#", unpack=True
            )
            self.sm_kf = self.sm_kfn
        ...
        #            pass
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/pheno/nlrh/" + param + "-SM.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/pheno/nlrh/" + param + "-NM.dat"
            )
            if nuda.env.verb:
                print("Reads file1:", file_in1)
            if nuda.env.verb:
            ...
            self.note = "write here notes about this EOS."
            self.sm_den, self.sm_kfn, self.sm_e2a, self.sm_pre, self.sm_cs2 = (
                np.loadtxt(file_in1, usecols=(0, 1, 2, 3, 4), comments="#", unpack=True)
            )
            self.nm_den, self.nm_kfn, self.nm_e2a, self.nm_pre, self.nm_cs2 = (
                np.loadtxt(file_in2, usecols=(0, 1, 2, 3, 4), comments="#", unpack=True)
            )
            self.sm_kf = self.sm_kfn
            #
        elif model.lower() == "ddrh":
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/pheno/ddrh/" + param + "-SM.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/pheno/ddrh/" + param + "-NM.dat"
            )
            if nuda.env.verb:
                print("Reads file1:", file_in1)
            if nuda.env.verb:
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/pheno/ddrhf/" + param + "-SM.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/pheno/ddrhf/" + param + "-NM.dat"
            )
            if nuda.env.verb:
                print("Reads file1:", file_in1)
            if nuda.env.verb:
    ...
    def print_outputs(self):
        ...
        if any(self.sm_den):
            print(f"   sm_den: {np.round(self.sm_den,2)} in {self.den_unit}")
        if any(self.sm_kfn):
            print(f"   sm_kfn: {np.round(self.sm_kfn,2)} in {self.kfn_unit}")
        if any(self.sm_e2a):
            print(f"   sm_e2a: {np.round(self.sm_e2a,2)} in {self.e2a_unit}")
        if any(self.nm_den):
            print(f"   nm_den: {np.round(self.nm_den,2)} in {self.den_unit}")
        if any(self.nm_kfn):
            print(f"   nm_kfn: {np.round(self.nm_kfn,2)} in {self.kfn_unit}")
        if any(self.nm_e2a):
            print(f"   nm_e2a: {np.round(self.nm_e2a,2)} in {self.e2a_unit}")
        if any(self.nm_gap):
            print(f"   nm_gap: {np.round(self.nm_gap,2)} in {self.gap_unit}")
        #
        if nuda.env.verb:

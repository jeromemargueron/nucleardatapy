    ...
    **Attributes:**
    ...
    def __init__(self, model="Skyrme", param="SLY5"):
        ...
        if model.lower() == "skyrme":
            ...
            name = np.loadtxt(
                file_in, usecols=(0), comments="#", unpack=True, dtype=str
            )
            (
                nsat,
                Esat,
            ...
            if param in name:
                ...
                self.nsat = nsat[ind][0]
                self.Esat = Esat[ind][0]
                self.Ksat = Ksat[ind][0]
                self.Qsat = Qsat[ind][0]
                self.Zsat = Zsat[ind][0]
                self.Esym = Esym[ind][0]
                self.Lsym = Lsym[ind][0]
                self.Ksym = Ksym[ind][0]
                self.Qsym = Qsym[ind][0]
                self.Zsym = Zsym[ind][0]
                self.msat = msat[ind][0]
                self.kappas = kappas[ind][0]
                self.kappav = kappav[ind][0]
                self.kappasym = kappasym[ind][0]
                self.Dmsat = Dmsat[ind][0]
            else:
                self.nep = False
        #
        elif model.lower() == "skyrme2":
            ...
            nsat, Esat, Ksat, Qsat, msat, Esym, Lsym, kappav = np.loadtxt(
                file_in, usecols=(1, 2, 3, 4, 5, 6, 7, 8), comments="#", unpack=True
            )
            kappas = 1.0 / msat - 1.0
            kappasym = kappas - kappav
            Dmsat = -2 * kappasym / ((1 + kappas) ** 2 - kappasym**2)
            ...
            nsat, Esat, Ksat, Qsat, Esym, Lsym, Ksym, msat = np.loadtxt(
                file_in, usecols=(1, 2, 3, 4, 5, 6, 7, 8), comments="#", unpack=True
            )
            kappas = 1.0 / msat - 1.0
            # kappasym = kappas - kappav

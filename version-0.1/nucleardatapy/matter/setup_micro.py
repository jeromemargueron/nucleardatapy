...
def micro_models_mb_matter(mb, matter):
    ...
    for j, model in enumerate(models):
        if (
            mb in model
            and "2BF" not in model
            and (matter.upper() in model or "AM" in model)
        ):
...
<<<<<<<< HEAD:version-0.1/nucleardatapy/matter/setup_micro.py
...
>>>>>>>> slalit:build/lib/nucleardatapy/matter/setup_micro-old.py
            #
...
>>>>>>>> slalit:build/lib/nucleardatapy/matter/setup_micro-old.py
            ...
            self.marker = '.'
            self.nm_kfn, nm_gap, self.nm_chempot, self.nm_effmass \
...
=======
        ...
        if model.lower() == "1981-var-am-fp":
            ...
            exit()
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/micro/2006-BHF-NM.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/micro/2006-BHF-SM.dat"
            )
            if nuda.env.verb:
                print("Reads file:", file_in1)
            if nuda.env.verb:
...
=======
            ...
            self.nm_kfn_fit = nuda.kf_n(self.nm_den_fit)
            # energy in NM
            self.nm_e2a_fit = func_GCR_e2a(
                self.nm_den_fit, a[k - 1], alfa[k - 1], b[k - 1], beta[k - 1]
            )
            self.nm_e2a_fit_err = np.abs(
                uncertainty_stat(self.nm_den_fit) * self.nm_e2a_fit
            )
            self.nm_e2v_fit = self.nm_den_fit * self.nm_e2a_fit
            self.nm_e2v_fit_err = self.nm_den_fit * self.nm_e2a_fit_err
            # pressure in NM
            ...
            # enthalpy per particle
            self.nm_h2a_fit = (
                nuda.cst.mnuc2 + self.nm_e2a_fit + self.nm_pre_fit / self.nm_den_fit
            )
            # sound speed in NM
...
=======
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/micro/2020-MBPT-SM-DHSL59.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/micro/2020-MBPT-NM-DHSL59.dat"
            )
            if nuda.env.verb:
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/micro/2020-MBPT-SM-DHSL69.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/micro/2020-MBPT-NM-DHSL69.dat"
            )
            if nuda.env.verb:
                print("Reads file1:", file_in1)
            if nuda.env.verb:
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "matter/micro/2023-MBPT-SM.csv"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "matter/micro/2023-MBPT-NM.csv"
            )
            if nuda.env.verb:
                print("Reads file1:", file_in1)
            if nuda.env.verb:
...
=======
            ...
            for k in range(100):
                ...
                af = func_e2a_NLEFT2024(xdata, *param)
                for l, val in enumerate(af):
                    if val > self.sm_e2a_max[l]:
                        self.sm_e2a_max[l] = val
                    if val < self.sm_e2a_min[l]:
                        self.sm_e2a_min[l] = val
                self.sm_e2a_err = 0.5 * (self.sm_e2a_max - self.sm_e2a_min)
                # pre
                af = func_pre_NLEFT2024(xdata, self.sm_den, *param)
                for l, val in enumerate(af):
...
=======
            ...
            for k in range(100):
                ...
                af = func_e2a_NLEFT2024(xdata, *param)
                for l, val in enumerate(af):
                    if val > self.nm_e2a_max[l]:
                        self.nm_e2a_max[l] = val
                    if val < self.nm_e2a_min[l]:
                        self.nm_e2a_min[l] = val
                self.nm_e2a_err = 0.5 * (self.nm_e2a_max - self.nm_e2a_min)
                # pre
                af = func_pre_NLEFT2024(xdata, self.nm_den, *param)
                for l, val in enumerate(af):
...
=======
        ...
        if self.e2v is not None:
            print(f"   e2v: {np.round(self.e2v,3)} in {self.e2v_unit}")
        ...
        if self.nm_e2a_fit_err is not None:
            print(
                f"   nm_e2a_fit_err: {np.round(self.nm_e2a_fit_err,3)} in {self.e2a_unit}"
            )

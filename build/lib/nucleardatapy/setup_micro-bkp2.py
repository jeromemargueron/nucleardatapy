...
def func_GCR_cs2(den, a, alfa, b, beta):
    dp_dn = (
        a * alfa * (alfa + 1.0) * (den / nsat) ** alfa
        + b * beta * (beta + 1.0) * (den / nsat) ** beta
    )
    enth = (
        mnuc2
...
class SetupMicro:
    ...
    **Attributes:**
    ...
    def __init__(self, model="1998-VAR-AM-APR"):
        ...
        if model.lower() == "1981-var-am-fp":
            ...
            file_in1 = os.path.join(
                nuda.param.path_data, "eos/micro/1981-VAR-NM-FP.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "eos/micro/1981-VAR-SM-FP.dat"
            )
            if nuda.env.verb:
            ...
            self.nm_den_min = min(self.nm_den)
            self.nm_den_max = max(self.nm_den)
            ...
            self.nm_pre = (
                nuda.cst.three * self.nm_kfn * self.nm_den * cs_nm_e2a(self.nm_kfn, 1)
            )
            ...
            self.sm_pre = (
                nuda.cst.three * self.sm_kfn * self.sm_den * cs_sm_e2a(self.sm_kfn, 1)
            )
            ...
            self.esym_e2a = cs_nm_e2a(2**nuda.cst.third * self.esym_kf) - cs_sm_e2a(
                self.esym_kf
            )
            self.esym_e2a_err = np.sqrt(
                cs_nm_e2a_err(2**nuda.cst.third * self.esym_kf) ** 2
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "eos/micro/1998-VAR-NM-APR.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "eos/micro/1998-VAR-SM-APR.dat"
            )
            if nuda.env.verb:
                print("Reads file:", file_in1)
            if nuda.env.verb:
            ...
            # self.marker = None
            self.marker = "."
            self.nm_kfn, self.nm_gap, self.nm_chempot, self.nm_effmass = np.loadtxt(
                file_in, usecols=(0, 1, 2, 3), unpack=True
            )
            ...
            #
            # pressure
            self.nm_pre = np.array(self.nm_den) * np.array(self.nm_chempot) - np.array(
                self.nm_e2v
            )
            ...
            file_in = os.path.join(
                nuda.param.path_data, "eos/micro/2008-QMC-NM-swave.dat"
            )
            if nuda.env.verb:
                print("Reads file:", file_in)
            ...
            self.nm_pre = (
                nuda.cst.three * self.nm_kfn * self.nm_den * cs_e2a(self.nm_kfn, 1)
            )
            y_err = np.insert(self.nm_e2a_err, 0, 0.0)
            cs_nm_e2a_err = CubicSpline(x, y_err)
            self.nm_pre_err = (
            ...
            self.linestyle = "solid"
            self.err = True
            self.nm_kfn, self.nm_e2a, self.nm_e2a_err = np.loadtxt(
            ...
            #
            file_in = os.path.join(
                nuda.param.path_data, "eos/micro/2010-QMC-NM-AV4.dat"
            )
            if nuda.env.verb:
                print("Reads file:", file_in)
            self.ref = "A. Gezerlis and J. Carlson PRC 81, 025803 (2010)"
            ...
            #
            file_in = os.path.join(
                nuda.param.path_data, "eos/micro/2010-NM-Hebeler.dat"
            )
            if nuda.env.verb:
                print("Reads file:", file_in)
            ...
            self.nm_kfn = nuda.kf_n(self.nm_den)
            # energy in NM
            self.nm_e2a = func_GCR_e2a(
                self.nm_den, a[k - 1], alfa[k - 1], b[k - 1], beta[k - 1]
            )
            self.nm_e2a_err = np.abs(uncertainty_stat(self.nm_den) * self.nm_e2a)
            self.nm_e2v = self.nm_den * self.nm_e2a
            self.nm_e2v_err = self.nm_den * self.nm_e2a_err
            # pressure in NM
            self.nm_pre = func_GCR_pre(
                self.nm_den, a[k - 1], alfa[k - 1], b[k - 1], beta[k - 1]
            )
            # chemical potential
            self.nm_chempot = (self.nm_pre + self.nm_e2v) / self.nm_den
            # sound speed in NM
            self.nm_cs2 = func_GCR_cs2(
                self.nm_den, a[k - 1], alfa[k - 1], b[k - 1], beta[k - 1]
            )
            #
            ...
            self.linestyle = "solid"
            self.nm_den, self.nm_e2a_2bf, self.nm_e2a_23bf = np.loadtxt(
                file_in, usecols=(0, 1, 2), unpack=True
            )
            self.nm_kfn = nuda.kf_n(self.nm_den)
            self.nm_den_min = min(self.nm_den)
            ...
            self.linestyle = "solid"
            self.err = True
            self.nm_den, self.nm_e2a_low, self.nm_e2a_up = np.loadtxt(
                file_in, usecols=(0, 1, 2), unpack=True
            )
            self.nm_den_min = min(self.nm_den)
            self.nm_den_max = max(self.nm_den)
            ...
            for i in range(0, 11):
                ...
                if i < 10:
                    file_in = os.path.join(
                        ...
                        "eos/micro/2016-MBPT-AM/EOS_spec_4_beta_0." + str(i) + ".txt",
                    )
                if i == 10:
                    file_in = os.path.join(
                        nuda.param.path_data,
                        "eos/micro/2016-MBPT-AM/EOS_spec_4_beta_1.0.txt",
                    )
                if nuda.env.verb:
                    print("Reads file:", file_in)
            ...
            self.esym_e2a = cs_nm_e2a(self.esym_den) - cs_sm_e2a(self.esym_den)
            self.esym_e2a_err = np.sqrt(
                cs_nm_e2a_err(self.esym_den) ** 2 + cs_sm_e2a_err(self.esym_den) ** 2
            )
            # print('esym:',self.esym_e2a)
            #
            # chemical potential
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "eos/micro/2020-MBPT-SM-DHSL59.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "eos/micro/2020-MBPT-NM-DHSL59.dat"
            )
            if nuda.env.verb:
            ...
            cs_sm_e2a_err = CubicSpline(x, y_err)
            self.sm_pre = (
                nuda.cst.three * self.sm_kfn * self.sm_den * cs_sm_e2a(self.sm_den, 1)
            )
            # y_err = np.insert( self.sm_e2a_err, 0, 0.0 )
            # cs_sm_e2a_err = CubicSpline( x, y_err )
            # self.sm_pre_err = self.sm_den**2 * cs_sm_e2a_err( self.sm_den, 1 )
            ...
            #
            file_in1 = os.path.join(
                nuda.param.path_data, "eos/micro/2020-MBPT-SM-DHSL69.dat"
            )
            file_in2 = os.path.join(
                nuda.param.path_data, "eos/micro/2020-MBPT-NM-DHSL69.dat"
            )
            if nuda.env.verb:
                print("Reads file1:", file_in1)
            if nuda.env.verb:
            ...
            if model.lower() == "2024-bhf-am-2bf-av8p":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_Av8p2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_Av8p2BF.dat",
                )
                self.label = "BHF-2024-2BF-Av8p"
            elif model.lower() == "2024-bhf-am-2bf-av18":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_Av182BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_Av182BF.dat",
                )
                self.label = "BHF-2024-2BF-Av18"
            elif model.lower() == "2024-bhf-am-2bf-bonn":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_BONN2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_BONN2BF.dat",
                )
                self.label = "BHF-2024-2BF-Bonn"
            elif model.lower() == "2024-bhf-am-2bf-cdbonn":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_CDBONN2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_CDBONN2BF.dat",
                )
                self.label = "BHF-2024-2BF-CDBonn"
            elif model.lower() == "2024-bhf-am-2bf-sscv14":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_SSCV142BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_SSCV142BF.dat",
                )
                self.label = "BHF-2024-2BF-SSCV14"
            elif model.lower() == "2024-bhf-am-2bf-nsc97a":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_NSC97a2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_NSC97a2BF.dat",
                )
                self.label = "BHF-2024-2BF-NSC97a"
            elif model.lower() == "2024-bhf-am-2bf-nsc97b":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_NSC97b2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_NSC97b2BF.dat",
                )
                self.label = "BHF-2024-2BF-NSC97b"
            elif model.lower() == "2024-bhf-am-2bf-nsc97c":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_NSC97c2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_NSC97c2BF.dat",
                )
                self.label = "BHF-2024-2BF-NSC97c"
            elif model.lower() == "2024-bhf-am-2bf-nsc97d":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_NSC97d2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_NSC97d2BF.dat",
                )
                self.label = "BHF-2024-2BF-NSC97d"
            elif model.lower() == "2024-bhf-am-2bf-nsc97e":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_NSC97e2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_NSC97e2BF.dat",
                )
                self.label = "BHF-2024-2BF-NSC97e"
            elif model.lower() == "2024-bhf-am-2bf-nsc97f":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-2BF/spin_isosp_NSC97f2BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-2BF/spin_isosp_NSC97f2BF.dat",
                )
            ...
            # 2+3BF
                ...
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_Av8p23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_Av8p23BF.dat",
                )
                self.label = "BHF-2024-23BF-Av8p"
            elif model.lower() == "2024-bhf-am-23bf-av18":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_Av1823BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_Av1823BF.dat",
                )
                self.label = "BHF-2024-23BF-Av18"
            elif model.lower() == "2024-bhf-am-23bfmicro-av18":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_Av1823BFmicro.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_Av1823BFmicro.dat",
                )
                self.label = "BHF-2024-23BFmicro-Av18"
            elif model.lower() == "2024-bhf-am-23bf-bonn":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_BONN23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_BONN23BF.dat",
                )
                self.label = "BHF-2024-23BF-Bonn"
            elif model.lower() == "2024-bhf-am-23bfmicro-bonnb":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_BONNB23BFmicro.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_BONNB23BFmicro.dat",
                )
                self.label = "BHF-2024-23BFMicro-BonnB"
            elif model.lower() == "2024-bhf-am-23bf-cdbonn":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_CDBONN23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_CDBONN23BF.dat",
                )
                self.label = "BHF-2024-23BF-CDBonn"
            elif model.lower() == "2024-bhf-am-23bf-sscv14":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_SSCV1423BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_SSCV1423BF.dat",
                )
                self.label = "BHF-2024-23BF-SSCV14"
            elif model.lower() == "2024-bhf-am-23bfmicro-nsc93":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_NSC9323BFmicro.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_NSC9323BFmicro.dat",
                )
                self.label = "BHF-2024-23BFmicro-NSC93"
            elif model.lower() == "2024-bhf-am-23bf-nsc97a":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_NSC97a23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_NSC97a23BF.dat",
                )
                self.label = "BHF-2024-23BF-NSC97a"
            elif model.lower() == "2024-bhf-am-23bf-nsc97b":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_NSC97b23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_NSC97b23BF.dat",
                )
                self.label = "BHF-2024-23BF-NSC97b"
            elif model.lower() == "2024-bhf-am-23bf-nsc97c":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_NSC97c23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_NSC97c23BF.dat",
                )
                self.label = "BHF-2024-23BF-NSC97c"
            elif model.lower() == "2024-bhf-am-23bf-nsc97d":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_NSC97d23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_NSC97d23BF.dat",
                )
                self.label = "BHF-2024-23BF-NSC9d7"
            elif model.lower() == "2024-bhf-am-23bf-nsc97e":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_NSC97e23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_NSC97e23BF.dat",
                )
                self.label = "BHF-2024-23BF-NSC97e"
            elif model.lower() == "2024-bhf-am-23bf-nsc97f":
                file_in1 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-SM-23BF/spin_isosp_NSC97f23BF.dat",
                )
                file_in2 = os.path.join(
                    nuda.param.path_data,
                    "eos/micro/2024-BHF-NM-23BF/spin_isosp_NSC97f23BF.dat",
                )
    ...
    def print_outputs(self):
        ...
        if self.sm_e2v is not None:
            print(f"   sm_e2v: {np.round(self.sm_e2v,3)} in {self.e2v_unit}")
        if self.sm_e2v_err is not None:
        ...
        if self.nm_e2v is not None:
            print(f"   nm_e2v: {np.round(self.nm_e2v,3)} in {self.e2v_unit}")
        if self.nm_e2v_err is not None:
            print(f"   nm_e2v_err: {np.round(self.nm_e2v_err,3)} in {self.e2v_unit}")
        if self.nm_pre is not None:
            print(f"   nm_pre: {np.round(self.nm_pre,3)} in {self.pre_unit}")
        #
        if self.esym_den is not None:
            print(f"   esym_den: {np.round(self.esym_den,3)} in {self.den_unit}")
        if self.esym_kf is not None:
            print(f"   esym_kf: {np.round(self.esym_kf,3)} in {self.kf_unit}")
        if self.esym_e2a is not None:
            print(f"   esym_e2a: {np.round(self.esym_e2a,3)} in {self.e2a_unit}")
        #
        if self.nm_gap is not None:
            print(f"   nm_gap: {np.round(self.nm_gap,3)} in {self.gap_unit}")
        if self.nm_gap_err is not None:
            print(f"   nm_gap_err: {np.round(self.nm_gap_err,3)} in {self.gap_unit}")
        #
        if nuda.env.verb:

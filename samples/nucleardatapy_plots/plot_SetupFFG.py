...
def main():
    ...
    #
    axs[0,0].set_ylabel(r'$E/E_{FFG}$')
    axs[0,0].set_xlim([0, 0.3])
    axs[0,0].set_ylim([0, 1.0])
    #
    axs[1,0].set_xlabel(r'n (fm$^{-3}$)')
    axs[1,0].set_ylabel(r'$E/A$ (MeV)')
    ...
    #
    ffg0 = nuda.SetupFFG( den = den, delta = delta0 )
    ffg1 = nuda.SetupFFG( den = den, delta = delta1 )
    if any(ffg0.e2a): 
    ...
    if any(ffg1.e2a): 
        axs[0,0].plot( ffg1.den, ffg1.e2a, linestyle='solid', label=ffg1.label )
        axs[1,0].plot( ffg1.den, ffg1.pre, linestyle='solid', label=ffg1.label )
        axs[0,1].plot( ffg1.kf_n, ffg1.e2a, linestyle='solid', label=ffg1.label )
        axs[1,1].plot( ffg1.kf_n, ffg1.pre, linestyle='solid', label=ffg1.label )
    ffg0.print_outputs( )
    #

        ...
        for model in models:
            ...
            if micro.nm_pre is not None:
                ...
                if mb in mb_check:
                    if micro.marker:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_den, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=None, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.nm_den, micro.nm_pre, marker=micro.marker, linestyle=None, markevery=micro.every, color=nuda.param.col[k] )
                    else:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_den, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=micro.linestyle, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.nm_den, micro.nm_pre, marker=micro.marker, linestyle=micro.linestyle, markevery=micro.every, color=nuda.param.col[k] )
                else:
                    mb_check.append(mb)
                    k += 1
                    if micro.marker:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_den, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=None, label=mb, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.nm_den, micro.nm_pre, marker=micro.marker, linestyle=None, label=mb, markevery=micro.every, color=nuda.param.col[k] )
                    else:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_den, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=micro.linestyle, label=mb, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( enm.nm_den, enm.nm_pre, marker=enm.marker, linestyle=micro.linestyle, label=mb, markevery=micro.every, color=nuda.param.col[k] )
        ...
        for param in params:
            ...
            if pheno.nm_pre is not None: 
                ...
                if model in model_check:
                    axs[1].plot( pheno.nm_den, pheno.nm_pre, color=nuda.param.col[k] )
                else:
                    model_check.append(model)
                    k += 1
                    axs[1].plot( pheno.nm_den, pheno.nm_pre, color=nuda.param.col[k], label=model )
                #pheno.label=None

        ...
        for model in models:
            ...
            if micro.sm_e2a is not None:
                ...
                if mb in mb_check:
                    if micro.marker:
                        if micro.e_err:
                            axs[0].errorbar( micro.sm_den, micro.sm_e2a, yerr=micro.sm_e2a_err, marker=micro.marker, linestyle=None, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, micro.sm_e2a, marker=micro.marker, linestyle=None, markevery=micro.every, color=nuda.param.col[k] )
                    else:
                        if micro.e_err:
                            axs[0].errorbar( micro.sm_den, micro.sm_e2a, yerr=micro.sm_e2a_err, marker=micro.marker, linestyle=micro.linestyle, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, micro.sm_e2a, marker=micro.marker, linestyle=micro.linestyle, markevery=micro.every, color=nuda.param.col[k] )
                else:
                    mb_check.append(mb)
                    k += 1
                    if micro.marker:
                        if micro.e_err:
                            axs[0].errorbar( micro.sm_den, micro.sm_e2a, yerr=micro.sm_e2a_err, marker=micro.marker, linestyle=None, label=mb, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, micro.sm_e2a, marker=micro.marker, linestyle=None, label=mb, markevery=micro.every, color=nuda.param.col[k] )
                    else:
                        if micro.e_err:
                            axs[0].errorbar( micro.sm_den, micro.sm_e2a, yerr=micro.sm_e2a_err, marker=micro.marker, linestyle=micro.linestyle, label=mb, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, micro.sm_e2a, marker=micro.marker, linestyle=micro.linestyle, label=mb, markevery=micro.every, color=nuda.param.col[k] )
        ...
        for param in params:
            ...
            if pheno.sm_e2a is not None: 
                ...
                if model in model_check:
                    axs[1].plot( pheno.sm_den, pheno.sm_e2a, color=nuda.param.col[k] )
                else:
                    model_check.append(model)
                    k += 1
                    axs[1].plot( pheno.sm_den, pheno.sm_e2a, color=nuda.param.col[k], label=model )
                #pheno.label=None

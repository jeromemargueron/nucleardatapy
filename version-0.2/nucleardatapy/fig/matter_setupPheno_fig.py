...
def matter_setupPheno_E_fig(pname, model, band):
    ...
    fig.subplots_adjust(
        left=0.15, bottom=0.12, right=None, top=0.88, wspace=0.05, hspace=0.05
    )
    #
        ...
        if matter.lower() == "nm":
            if any(pheno.nm_e2a):
                if model == "Skyrme" and check.isInside:
                    axs[0, 0].plot(
                        pheno.nm_den, pheno.nm_e2a, linestyle=lstyle, label=pheno.label
                    )
                ...
                axs[1, 0].plot(
                    pheno.nm_den,
                    pheno.nm_e2a / nuda.effg_nr(pheno.nm_kfn),
                    linestyle=lstyle,
                )
                axs[1, 1].plot(
                    pheno.nm_kfn,
            ...
            if any(pheno.sm_e2a):
                if model == "Skyrme" and check.isInside:
                    axs[0, 0].plot(
                        pheno.sm_den, pheno.sm_e2a, linestyle=lstyle, label=pheno.label
                    )
                elif model == "Skyrme" and check.isOutside:
                ...
                axs[1, 0].plot(
                    pheno.sm_den,
                    pheno.sm_e2a / nuda.effg_nr(pheno.sm_kf),
                    linestyle=lstyle,
                )
                axs[1, 1].plot(
                    pheno.sm_kf,

...
def eos_setupAM_e_asy_lep_fig( pname, micro_mbs, pheno_models, asy ):
    """
    ...
    for kmb,mb in enumerate(micro_mbs):
        ...
        for model in models:
            ...
            if micro.esym is not None: 
                ...
                if mb in mb_check:
                    axs[0].plot( micro.den, micro.e2a_lep, marker='o', linestyle=micro.linestyle, markevery=micro.every, color=nuda.param.col[kmb] )
                else:
                    mb_check.append(mb)
                    axs[0].plot( micro.den, micro.e2a_lep, marker='o', linestyle=micro.linestyle, label=mb, markevery=micro.every, color=nuda.param.col[kmb] )
            #
        ...
        for param in params:
            ...
            if pheno.e2a_lep is not None: 
                ...
                if model in model_check:
                    axs[1].plot( pheno.den, pheno.e2a_lep, linestyle=pheno.linestyle, markevery=pheno.every, color=nuda.param.col[kmodel] )
                else:
                    model_check.append(model)
                    axs[1].plot( pheno.den, pheno.e2a_lep, linestyle=pheno.linestyle, label=model, markevery=pheno.every, color=nuda.param.col[kmodel] )
            #                    

...
def matter_setupHIC_fig(pname, constraints):
    ...
    fig.subplots_adjust(
        left=0.10, bottom=0.12, right=None, top=0.98, wspace=0.25, hspace=0.05
    )
    #
    ...
    for constraint in constraints:
        ...
        if hic.sym_enr_isodiff is not None:
            axs[1, 1].errorbar(
                hic.den_isodiff,
                hic.sym_enr_isodiff,
                xerr=hic.den_isodiff_err,
                yerr=hic.sym_enr_isodiff_err,
                fmt="o",
                label=hic.label,
                color=hic.color,
                capsize=2,
        ...
        if hic.sym_enr_spirit is not None:
            axs[1, 1].errorbar(
                hic.den_spirit,
                hic.sym_enr_spirit,
                xerr=hic.den_spirit_err,
                yerr=hic.sym_enr_spirit_err,
                fmt="o",
                label=hic.label,
                color=hic.color,
                capsize=2,

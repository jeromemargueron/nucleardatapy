...
def matter_preSM_fig( pname, micro_mbs, pheno_models, band ):
        ...
        for model in models:
            ...
            if micro.sm_pre is not None:
                ...
                if mb in mb_check:
                    if micro.marker:
                        if micro.p_err:
                            axs[0].errorbar( micro.sm_den, micro.sm_pre, yerr=micro.sm_pre_err, marker=micro.marker, linestyle=None, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, micro.sm_pre, marker=micro.marker, linestyle=None, markevery=micro.every, color=nuda.param.col[k] )
                    else:
                        if micro.p_err:
                            axs[0].errorbar( micro.sm_den, micro.sm_pre, yerr=micro.sm_pre_err, marker=micro.marker, linestyle=micro.linestyle, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, micro.sm_pre, marker=micro.marker, linestyle=micro.linestyle, markevery=micro.every, color=nuda.param.col[k] )
                else:
                    mb_check.append(mb)
                    k += 1
                    if micro.marker:
                        if micro.p_err:
                            axs[0].errorbar( esm.sm_den, micro.sm_pre, yerr=micro.sm_pre_err, marker=micro.marker, linestyle=None, label=mb, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, micro.sm_pre, marker=micro.marker, linestyle=None, label=mb, markevery=micro.every, color=nuda.param.col[k] )
                    else:
                        if micro.p_err:
                            axs[0].errorbar( micro.sm_den, micro.sm_pre, yerr=micro.sm_pre_err, marker=micro.marker, linestyle=micro.linestyle, label=mb, errorevery=micro.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( micro.sm_den, esm.sm_pre, marker=esm.marker, linestyle=micro.linestyle, label=mb, markevery=micro.every, color=nuda.param.col[k] )
    ...
    #axs[0].plot( band.den, (band.e2a+band.e2a_std), color='k', linestyle='dashed' )
    axs[0].text(0.06,60,'microscopic models',fontsize='10')
        ...
        for param in params:
            ...
            if pheno.sm_pre is not None: 
                ...
                if model in model_check:
                    axs[1].plot( pheno.sm_den, pheno.sm_pre, color=nuda.param.col[k] )
                else:
                    model_check.append(model)
                    k += 1
                    axs[1].plot( pheno.sm_den, pheno.sm_pre, color=nuda.param.col[k], label=model )
                #pheno.label=None
    ...
    #axs[1].plot( band.den, (band.e2a+band.e2a_std), color='k', linestyle='dashed' )
    axs[1].text(0.06,60,'phenomenological models',fontsize='10')

...
def matter_eosNM_fig( pname, micro_mbs, pheno_models, band ):
    """
        ...
        for model in models:
            ...
            if micro.nm_pre is not None:
                ...
                if mb in mb_check:
                    if micro.marker:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_e2v, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=lstyle, errorevery=micro.every, color=nuda.param.col[kmb] )
                        else:
                            axs[0].plot( micro.nm_e2v, micro.nm_pre, marker=micro.marker, linestyle=lstyle, markevery=micro.every, color=nuda.param.col[kmb] )
                    else:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_e2v, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=lstyle, errorevery=micro.every, color=nuda.param.col[kmb] )
                        else:
                    ...
                    if micro.marker:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_e2v, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=lstyle, label=mb, errorevery=micro.every, color=nuda.param.col[kmb] )
                        else:
                            axs[0].plot( micro.nm_e2v, micro.nm_pre, marker=micro.marker, linestyle=lstyle, label=mb, markevery=micro.every, color=nuda.param.col[kmb] )
                    else:
                        if micro.p_err:
                            axs[0].errorbar( micro.nm_e2v, micro.nm_pre, yerr=micro.nm_pre_err, marker=micro.marker, linestyle=lstyle, label=mb, errorevery=micro.every, color=nuda.param.col[kmb] )
                        else:
                            axs[0].plot( enm.nm_e2v, enm.nm_pre, marker=enm.marker, linestyle=lstyle, label=mb, markevery=micro.every, color=nuda.param.col[kmb] )
            # end of model
        # end of mb

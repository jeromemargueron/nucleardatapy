...
def matter_setupFFGNuc_EP_fig(
    pname, mss=[1.0], den=np.linspace(0.01, 0.35, 10), kfn=np.linspace(0.5, 2.0, 10)
):
    """
    ...
    fig.subplots_adjust(
        left=0.12, bottom=0.12, right=None, top=0.9, wspace=0.05, hspace=0.05
    )
    #
    ...
    for ims, ms in enumerate(mss):
        ...
        if any(ffg0.e2a_int_nr):
            ...
            axs[0, 1].plot(
                ffg0kf.kf_n, ffg0kf.e2a_int, linestyle="solid", color=nuda.param.col[0]
            )
            axs[0, 1].plot(
                ffg0kf.kf_n,
                ffg0kf.e2a_int_nr,
            ...
            axs[1, 1].plot(
                ffg0kf.kf_n, ffg0kf.pre, linestyle="solid", color=nuda.param.col[0]
            )
            axs[1, 1].plot(
                ffg0kf.kf_n,
                ffg0kf.pre_nr,
        ...
        if any(ffg1.e2a_int_nr):
            ...
            axs[0, 1].plot(
                ffg1kf.kf_n, ffg1kf.e2a_int, linestyle="dashed", color=nuda.param.col[1]
            )
            axs[0, 1].plot(
                ffg1kf.kf_n,
                ffg1kf.e2a_int_nr,
            ...
            axs[1, 1].plot(
                ffg1kf.kf_n, ffg1kf.pre, linestyle="dashed", color=nuda.param.col[1]
            )
            axs[1, 1].plot(
                ffg1kf.kf_n,
                ffg1kf.pre_nr,
    ...
    for ims, ms in enumerate(mss):
        ...
        if any(ffg0.e2a_int_nr):
            ...
            if ims == 0:
                ...
                axs[0].plot(
                    ffg0.e2v, ffg0.pre, linestyle="solid", color=nuda.param.col[0]
                )
                axs[0].plot(
                    ffg0.e2v,
                    ffg0.pre_nr,
        ...
        if any(ffg1.e2a_int_nr):
            ...
            if ims == 0:
                ...
                axs[0].plot(
                    ffg1.e2v, ffg1.pre, linestyle="dashed", color=nuda.param.col[1]
                )
                axs[0].plot(
                    ffg1.e2v,
                    ffg1.pre_nr,

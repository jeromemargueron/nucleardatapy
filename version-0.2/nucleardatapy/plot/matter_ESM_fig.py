        ...
        for model in models:
            ...
            if esm.sm_e2a is not None:
                ...
                if mb in mb_check:
                    if esm.marker:
                        if esm.err:
                            axs[0].errorbar( esm.den, esm.sm_e2a, yerr=esm.sm_e2a_err, marker=esm.marker, linestyle=None, errorevery=esm.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( esm.den, esm.sm_e2a, marker=esm.marker, linestyle=None, markevery=esm.every, color=nuda.param.col[k] )
                    else:
                        if esm.err:
                            axs[0].errorbar( esm.den, esm.sm_e2a, yerr=esm.sm_e2a_err, marker=esm.marker, linestyle=esm.linestyle, errorevery=esm.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( esm.den, esm.sm_e2a, marker=esm.marker, linestyle=esm.linestyle, markevery=esm.every, color=nuda.param.col[k] )
                else:
                    mb_check.append(mb)
                    k += 1
                    if esm.marker:
                        if esm.err:
                            axs[0].errorbar( esm.den, esm.sm_e2a, yerr=esm.esym_err, marker=esm.marker, linestyle=None, label=mb, errorevery=esm.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( esm.den, esm.sm_e2a, marker=esm.marker, linestyle=None, label=mb, markevery=esm.every, color=nuda.param.col[k] )
                    else:
                        if esm.err:
                            axs[0].errorbar( esm.den, esm.sm_e2a, yerr=esm.sm_e2a_err, marker=esm.marker, linestyle=esm.linestyle, label=mb, errorevery=esm.every, color=nuda.param.col[k] )
                        else:
                            axs[0].plot( esm.den, esm.esym, marker=esm.marker, linestyle=esm.linestyle, label=mb, markevery=esm.every, color=nuda.param.col[k] )
        ...
        for param in params:
            ...
            if esm.sm_e2a is not None: 
                ...
                if model in model_check:
                    axs[1].plot( esm.den, esm.sm_e2a, color=nuda.param.col[k] )
                else:
                    model_check.append(model)
                    k += 1
                    axs[1].plot( esm.den, esm.sm_e2a, color=nuda.param.col[k], label=model )
                #pheno.label=None

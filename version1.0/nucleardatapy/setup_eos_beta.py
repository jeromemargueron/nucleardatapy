    ...
    **Attributes:**
    ...
    def __init__( self, model = '1998-VAR-AM-APR', param = None, kind = 'micro', var1 = np.linspace(0.01,0.4,100) ):
        ...
        if kind == 'micro':
            ...
            eos = nuda.SetupEOSPheno( model = model, param = param )
            #eos.print_outputs( )
        self.label = eos.label
        self.every = eos.every
        self.linestyle = eos.linestyle
        self.marker = eos.marker
        #print('type esym:',type(eos.esym_e2a))

        #if isinstance(eos.esym_e2a, np.ndarray ):
        if eos.esym_e2a is not None:
            self.den = eos.esym_den
            self.esym = eos.esym_e2a
            self.x_e = []
            self.x_mu = []
            tmp1 = (4*eos.esym_e2a[0]/nuda.cst.hbc)**nuda.cst.three
            x_e = tmp1 / (3*nuda.cst.pi2*eos.esym_den[0] + 6*tmp1 )
            #x_e = 0.1
            x_mu = 0.0
            for ind,den in enumerate(eos.esym_den):
                x_e, x_mu =  fsolve(beta_eq, (x_e, x_mu), args=(den,eos.esym_e2a[ind]) )
                print(f' ind:{ind}, den:{den:.3f}, esym:{eos.esym_e2a[ind]:.0f}, x_e:{x_e:.3f}, x_mu:{x_mu:.3f}')
                self.x_e.append( x_e )

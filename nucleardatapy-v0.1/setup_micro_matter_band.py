...
class SetupMicroMatterBand():
    ...
    **Attributes:**
    ...
    def __init__( self, models = [ '2016-MBPT-AM' ], nden = 10, ne = 200, den=None, matter='NM', e2a_min = -20.0, e2a_max = 50.0 ):
        ...
        """
        #
        if nuda.env.verb: print("Enter SetupMicroMatterBand()")
        #
        if matter.lower() == 'nm':
            print('\nBand in NM')
            ...
            print('\nBand for Esym')
        #
        self = SetupMicroMatterBand.init_self( self )
        #
        #: Attribute model.
        self.models = models
        ...
        #
        modelsref, modelsref_lower = nuda.models_micro_matter()
        for model in models:
        ...
        if den is not None:
            ...
            for model in models:
                mic = nuda.SetupMicroMatter( model = model )
                if matter.lower() == 'nm':
                    den_min_tmp.append( mic.nm_den_min ); den_max_tmp.append( mic.nm_den_max )
                elif matter.lower() == 'sm':
                    den_min_tmp.append( mic.sm_den_min ); den_max_tmp.append( mic.sm_den_max )
                elif matter.lower() == 'esym':
                    den_min_tmp.append( mic.esym_den_min ); den_max_tmp.append( mic.esym_den_max )
        ...
        for model in models:
            ...
            if matter.lower() == 'nm':
                ...
                cs_e2a = CubicSpline( mic.esym_den, mic.esym_e2a )
                cs_e2a_err = CubicSpline( mic.esym_den, mic.esym_e2a_err )
        ...
        #
        if nuda.env.verb: print("Exit SetupMicroMatterBand()")
        #
